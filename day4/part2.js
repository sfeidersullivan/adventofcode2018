...
const frequentSleeper = Object.entries(map).sort((a,b) => {
  const aSum = Object.values(a[1]).reduce((accum, val) => val > accum ? val : accum, 0);
  const bSum = Object.values(b[1]).reduce((accum, val) => val > accum ? val : accum, 0);
  return bSum - aSum;
})

console.log(frequentSleeper[0]);
